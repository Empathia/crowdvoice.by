...
Class(CV, 'OrganizationProfileEditTab').inherits(Widget)({
  ...
  prototype: {
    ...
    _setup: function _setup() {
      ...
      if (this.data.entity.images.card && this.data.entity.images.card.url) {
        this.profileImage.setImage(this.data.entity.images.card.url);
      }
      ...
      if (this.data.entity.backgrounds.card && this.data.entity.backgrounds.card.url) {
        this.backgroundImage.setImage(this.data.entity.backgrounds.card.url);
      }
      ...
      this.appendChild(new CV.UI.Input({
        ...
        data: {
          label: 'Organization’s Profile Name',
          hint: '(65 characters max)',
          attr: {
            value: this.data.entity.profileName,
            type: 'text',
            maxLength: 65
          },
          inputClassName: '-md -block'
      ...
      })).render(this.mainElement).setValue(this.data.entity.description);
      ...
      this.appendChild(new CV.UI.Input({
        ...
        data: {
          ...
          hint: '',
          attr: {
            value: (this.data.entity.location === null ? '' :  this.data.entity.location),
            type: 'text'
          },
    ...
    _profileNameAvailabilityHandler: function _profileNameAvailabilityHandler(err, res) {
      ...
      if ((res.status === "taken") && (this.data.entity.profileName !== this.profilenameInput.getValue())) {
        return this.profilenameInput.clearState().error().updateHint({
