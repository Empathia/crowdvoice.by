...
var ImageDownloader = require(__dirname + '/image_downloader.js');
...
var FlickrDownloader = Module('FlickrDownloader').includes(Downloader)({
  ...
  retrieveData : function (url, done) {
    ...
    async.parallel([
      function (done) {
        var getInfo = request.get('https://api.flickr.com/services/rest/')
        getInfo.query({
      ...
      function (done) {
        var source = '';
        var getSizes = request.get('https://api.flickr.com/services/rest/')
        getSizes.query({
        ...
        getSizes.end(function (err, res) {
          ...
          // Download Image
          ImageDownloader.download(source, function (err, data) {
            photoData = data;
            done();
          });
        });
