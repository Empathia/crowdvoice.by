...
var ImageDownloader = Module('ImageDownloader').includes(Downloader)({
  ...
  retrieveData : function (url, check, done) {
    if (!done) { done = check; check = null; }
    ...
    request(url).on('response', function(response) {
      ...
      var req = client.get(longUrl, function (res) {
        if (res.headers['content-type'] && res.headers['content-type'].match('image')) {
          ...
          imagesize(res, function (err, size) {
            ...
            var sreq = client.get(longUrl, function (res) {
          ...
          logger.log('url is not an image (' + longUrl + ')')
          return done(new Error('URL is not an Image'));
        }
