  ...
  prototype: {
    hasImage: function (config) {
      ...
      var imageObject = {
        ...
        meta: function (versionName) {
          try {
            return JSON.parse(model[property + 'Info'])[versionName]
          } catch (e) {
            return {};
          }
        },
        processVersions: function processVersions (done) {
          model.uploadImage(property, model[property].url('original'), done);
        },
        versions: versions,
    ...
    uploadImage : function uploadImage (property, image, done) {
      ...
      async.each(Object.keys(versions), function (versionName, done) {
        ...
        model[property + 'Info'] = JSON.stringify(imagesMetadata);
