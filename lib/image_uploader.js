      ...
      async.each(Object.keys(versions), function (versionName, done) {
        ...
        var upload = function (readStream, imageInfo) {
          ...
          if (!global.useGM) {
            ...
            if (typeof(transform) === 'function') {
              var transformStream = transform(readStream);
              uploadParams.Body = transformStream;

              imagesMetadata[versionName] = {};
              gm(transformStream).size({ bufferStream: true }, function(err, size) {
                imagesMetadata[versionName].width = size.width;
                imagesMetadata[versionName].height = size.height;
              }).format({ bufferStream : true }, function(err, format) {
                imagesMetadata[versionName].format = format;
              });
            } else {
              done(new Error('Version is not a transform function'));
            }
