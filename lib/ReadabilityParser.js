  ...
  return tryRequest(SCRAPPER_SERVICE_URL + '/?u=' + url)
    .then(function(response) {
      ...
      if (data === null) {
        ...
        response = response.replace(/<(svg|noscript|script|style)[^<>]*>[\s\S]*?<\\?\/\1>/g, '');
        ...
        response = response.replace(/(\w+)>\s*<(\w+)/g, '$1>\n<$2');

        // remove all tags but safe and top-level tags
        response = stripTags(response, safe.concat(['body', 'html', 'head']), '\n');

        // normalize all white-space
        response = response.replace(/<(\w+)[^<>]*?>\s*<\/\1>/g, '\n');
