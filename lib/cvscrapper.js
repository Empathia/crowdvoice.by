  ...
  processUrl : function processUrl (url, done) {
    ...
    try {
      request(url, function(err, response, body) {
        var longUrl = response.request.url.href;

        if (err) { return done(err); }

        // Download raw data
        scrapper.downloadData(longUrl, function (err, data) {
          if (err) { done(err); return; }
          // Process raw data
          scrapper.processData(longUrl, data, function (err, info) {
            if (err) { done(err); return; }

            // Preprocess until 10 images
            var processedImages = [];
            async.each(info.images, function (image, done) {
              if (processedImages.length > 10) { return done(); }
