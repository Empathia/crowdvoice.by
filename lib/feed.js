var feed = {
  ...
  entityFollowsEntity: function (req, followEntityResult, next) {
    ...
    EntityFollower.find({ id: recordId }, function (err, firstResult) {
      ...
      // find all the followers of followed entity
      EntityFollower.find({ followed_id: firstResult[0].followerId }, function (err, secondResult) {
        if (err) { return next(err) }
        ...
        async.each(ids, function (val, callback) {
          action = new FeedAction({
            itemId: firstResult[0].followedId,
            itemType: 'entity',
  ...
  entityFollowsVoice: function (req, followVoiceResult, next) {
    ...
    // find the record
    VoiceFollower.find({ id: recordId }, function (err, firstResult) {
      if (err) { return next(err) }

      // find all the followers of the followed entity
      EntityFollower.find({ followed_id: firstResult[0].entityId }, function (err, secondResult) {
        if (err) { return next(err) }
        ...
        async.each(ids, function (val, callback) {
          action = new FeedAction({
            itemId: firstResult[0].voiceId,
            itemType: 'voice',
            actionDoer: firstResult[0].entityId,
            action: 'followed',
            followerId: val,
            read: false,
          })
