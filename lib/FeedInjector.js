            ...
            async.each(followerIds, function (followerId, next) {
              ...
              NotificationSetting.find({ entity_id: followerId }, function (err, setting) {
                ...
                async.series([
                  ...
                  // figure out which user to send the email to, because organizations
                  function (next) {
                    if (!setting[0].emailSettings[actionFunctionName]) {
                      return next()
                    }

                    Entity.find({ id: followerId }, function (err, entity) {
                      if (err) { return next(err) }

                      if (entity[0].type === 'organization') {
                        EntityOwner.find({ owned_id: followerId }, function (err, owner) {
                          if (err) { return next(err) }

                          User.find({ entity_id: owner[0].ownerId }, function (err, user) {
                            if (err) { return next(err) }
                  ...
                  function (next) {
                    ...
                    FeedPresenter.build([new FeedAction(actionObj)], null, function (err, presentedFeed) {
                      ...
                      return NotificationMailer.notification(emailUser, presentedFeed[0], next)
                    })
                  },
                ], next)
              })
