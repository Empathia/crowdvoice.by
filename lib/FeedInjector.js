...
require(path.join(__dirname, '../mailers/NotificationsMailer.js'))
          ...
          async.series([
            ...
            async.each(followerIds, function (followerId, next) {
              NotificationSetting.find({ entity_id: followerId }, function (err, setting) {
                if (!setting.settings[actionFunctionName]) {
                  return next()
                }
                ...
                async.series([
                  ...
                  function (next) {
                    User.find({ entity_id: followerId }, function (err, user) {
                      return NotificationMailer.notification(user[0], presentedAction, next)
                    })
                  },
                ], function (err) {
                  return next(err)
