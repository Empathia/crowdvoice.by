...
var FeedInjector = Class('FeedInjector')({
  ...
  prototype: {
    ...
    injectNotification: function (whoId, actionString, itemModel, callback) {
      ...
      this['_' + actionFunction].call(this, itemModel, whoId, function (err) {
        ...
        switch (actionFunction) {
          ...
          case 'notifNewRepost':
            emailName = 'newRepost'
            modelProperty = 'ownerId'
            settingName = 'selfNewRepost'
            break
        }
        ...
        async.series([
          ...
          function (nextSeries1) {
            async.series([
              function (nextSeries2) {
                if (modelProperty !== 'ownerId') {
                  return nextSeries2()
                }

                K.Entity.query()
                  .where('id', itemModel.ownerId)
          ...
          NotificationSetting.find({ entity_id: realReceiverEntity.id }, function (err, result) {
            ...
            } else if (emailName === 'newRepost') {
              return NotificationMailer[emailName]({
                entity: receiverEntity,
                realEntity: realReceiverEntity,
                user: receiverUser,
              }, reposter, itemModel, callback)
            }
          })
        })
      })
        ...
        async.series([
          ...
          function (nextSeries) {
            NotificationSetting.find({
              ...
              var settingName = actionFunctionName
    ...
    _notifNewRepost: function (repostedPostModel, whoId, callback) {
      this._createNotificationEntry('notifNewRepost', {
        itemType: 'post',
        itemId: repostedPostModel.id,
        action: 'reposted your post',
        who: whoId,
      }, repostedPostModel.ownerId, callback)
    },
