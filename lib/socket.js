...
module.exports = function (socket) {
  ...
  // On new notification
  Notification.bind('afterSave', function(data) {
  ...
  socket.on('getNotifications', function(data) {
    ...
    async.series([function(done) {
      ...
      Notification.find(['follower_id = ? AND read = false AND created_at > ? ORDER BY created_at ASC', [hashids.decode(currentPerson.id)[0], moment(new Date(currentPerson.lastNotificationDate).toISOString()).format()]], function(err, result) {
        if (err) {
          return done(err);
        }
      ...
      if (notifications.length > 0) {
        Entity.find({id : hashids.decode(currentPerson.id)[0]}, function(err, res) {
          ...
          entity.lastNotificationDate = notifications[0].createdAt;
