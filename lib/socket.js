module.exports = function (socket) {
  ...
  socket.on('getApprovedPostsPage', function(voiceId, page, up) {
    ...
    db.raw('SELECT * FROM (SELECT *, row_number() OVER (ORDER BY "Posts".published_at DESC) FROM "Posts" WHERE "Posts".voice_id = ? AND approved = true ORDER BY "Posts".published_at DESC LIMIT ?) AS posts WHERE row_number >= ?', [hashids.decode(voiceId)[0], VoicesController.POSTS_PER_PAGE, offset]).exec(function(err, posts) {
      posts = posts.rows;
  ...
  socket.on('getUnpprovedPostsPage', function(voiceId, page, up) {
    var offset = page * VoicesController.POSTS_PER_PAGE;

    db.raw('SELECT * FROM (SELECT *, row_number() OVER (ORDER BY "Posts".published_at DESC) FROM "Posts" WHERE "Posts".voice_id = ? AND approved = false ORDER BY "Posts".published_at DESC LIMIT ?) AS posts WHERE row_number >= ?', [hashids.decode(voiceId)[0], VoicesController.POSTS_PER_PAGE, offset]).exec(function(err, posts) {
      posts = posts.rows;
