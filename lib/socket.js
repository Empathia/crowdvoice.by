...
module.exports = function (socket) {
  ...
  socket.on('getSavedPostsPage', function (entityId, page, up) {
    ...
    return K.SavedPost.query()
      ...
      .from(function () {
        this
          .select(db.raw('*, row_number() over (order by "SavedPosts".created_at desc)'))
          .from('SavedPosts')
          .where('entity_id', hashids.decode(entityId)[0])
          .orderBy('SavedPosts.created_at', 'desc')
          .as('savedPosts')
      })
      ...
      .then(function (savedPosts) {
        logger.info(savedPosts.length);

        var posts = savedPosts.map(function (s) {
          return s.post;
        });

        return K.PostsPresenter.build(posts, socket.request.session.currentPerson)
          .then(function(results) {
            logger.info(posts.length, results.length);

            return socket.emit('savedPostsPage', results, page, up);
          });
      })
      .catch(function (err) {
        logger.error('Socket error: getSavedPostsPage');
        logger.error(err.stack);

        return socket.emit('getSavedPostsPage', { error: err }, page, up);
      });
  });
  ...
  socket.on('getApprovedPostsPage', function(voiceId, page, up) {
    ...
    return K.Post.query()
      ...
      .from(function () {
        this
          .select(db.raw('*, row_number() over (orxder by "Posts".published_at desc)'))
          .from('Posts')
  ...
  socket.on('getApprovedMonthPosts', function(voiceId, dateString, up) {
    ...
    logger.info(voiceId, dateString, up);
  ...
  var notificationHandler = function(data) {
    ...
    async.series([function(done) {
      ...
      currentPerson.id = hashids.encode(currentPerson.id)
