...
var postsPageQuery = function (info) {
  return K.Post.query()
    .select('*')
    .from(function () {
      this
        .select(K.Post.knex().raw('select *, row_number()'))
        .over(function () {
          this.orderBy('"Posts".published_at', 'desc')
        })
        .from('"Posts"')
        .where('"Posts".voice_id', '=', hashids.decode(info.voiceId)[0])
        .andWhere('approved', '=', info.approved)
        .orderBy('"Posts".published_at')
    })
    .as('posts')
    .where('row_number', '>', info.offset)
