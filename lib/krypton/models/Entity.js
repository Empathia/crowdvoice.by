    ...
    hasAccessToVoice: function (voice) {
      ...
      return new Promise(function (res, rej) {
        ...
        if (that.contributedVoices) {
          voices = voices.concat(that.)
        }

        if (that.organizations[0].voices) {
          voices = voices.concat.apply(this, that.organizations.map(function (o) {
            return o.voices
          }))
        }

        if (that.memberOrganizations[0].voices) {
          voices = voices.concat.apply(this, that.memberOrganizations.map(function (o) {
            return o.voices
          }))
        }
        ...
        if (voices.length < 1) {
          return rej(new Error('Couldn\'t find any voices'))
        }

        var matchingVoices = []

        matchingVoices = voices.filter(function (v) { return (v.id === voice.id) })
