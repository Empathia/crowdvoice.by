...
K.Post = Class(K, 'Post').inherits(Krypton.Model).includes(ImageUploader)({
  ...
  SOURCE_SERVICE_FACEBOOK: 'facebook',
  ...
  prototype: {
    ...
    init: function (config) {
      ...
      this.on('afterSave', function(done) {
        var published = false;

        K.Post.query()
          .where({'voice_id': model.voiceId})
          .count('*')
          .then(function(count) {
            count = count[0];
          ...
          .then(function() {
            ...
            return K.Voice.query()
              ...
              .then(function(voice) {
                ...
                if (voice.status === Voice.STATUS_ARCHIVED) {
                  return done();
                }

                voice.status = Voice.STATUS_PUBLISHED;
