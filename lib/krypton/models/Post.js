...
K.Post = Class(K, 'Post').inherits(Krypton.Model).includes(ImageUploader)({
  ...
  prototype: {
    ...
    init: function (config) {
      ...
      this.on('beforeDestroy', function (callback) {
        if (!model.repostInfo.originalPostId) {
          return callback()
        }

        K.Post.query()
          .whereRaw('cast(repostInfo->>\'originalPostId\' as integer) = ?', model.repostInfo.originalPostId)
          .then(function (posts) {
            var ids = posts.map(function (p) { return p.id }),
              newRepostId = ids.shift()

            return K.Post.query()
              .update('repostInfo', { originalPostId: newRepostId })
              .where('id', 'in', ids)
              .then(function (updatedRows) {
                return K.Post.query()
                  .update('repostInfo', {})
                  .where('id', newRepostId)
              })
          })
          .then(callback)
          .catch(callback)
