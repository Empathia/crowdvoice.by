...
K.Message = Class(K, 'Message').inherits(Krypton.Model)({
  ...
  prototype: {
    ...
    isPersonSender: function (person) {
      var that = this

      return new Promise(function (res, rej) {
        if (!that.id) {
          throw new Error('K.Message doesn\'t have an ID')
        }

        return K.Entity.query()
          .where('id', person.id)
          .include('organizations')
          .then(function (fetchEntity) {
            var ownedIds = fetchEntity[0].organizations.map(function (o) {
              return o.id
            })
            ownedIds.push(fetchEntity[0].id)

            var isMsgSender = (ownedIds.indexOf(that.senderEntityId) !== -1)

            return res(isMsgSender)
          })
          .catch(rej)
      })
    }
