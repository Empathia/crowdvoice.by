K.ThreadsPresenter = Module(K, 'ThreadsPresenter')({
  ...
  build: function (threads, currentPerson, preserveHidden) {
    return new Promise(function (resolve, reject) {
      if (!currentPerson) {
        throw new Error('K.ThreadsPresenter requires the \'currentPerson\' argument')
      }

      preserveHidden = preserveHidden || false
        ...
        Promise.resolve()
          ...
          .then(function () {
            ...
            if (!preserveHidden) {
              ...
              filteredMessages = thread.messages.filter(function (m) {
                return !m['hiddenFor' + (m.isPersonSender(fetchCurrentPerson.id) ? 'Sender' : 'Receiver')]
              })
            } else {
              filteredMessages = thread.messages
            }

            return K.MessagesPresenter.build(filteredMessages, currentPerson)
              .then(function (pres) {
