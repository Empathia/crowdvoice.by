  ...
  prototype : {
    ...
    fetchTweets : function(callback) {
      ...
      async.series([function(next) {
        ...
        }).orderBy('created_at', 'DESC').first('id_str').exec(function(err, result) {
          if (err) {
            return next(err);
          }
        ...
        logger.log('Fetching ' + fetcher.count + ' tweets');
        ...
        TwitterClient.get(
          ...
          function(err, tweets, response) {
            ...
            logger.log('Fetched ' +  tweets.statuses.length + ' tweets...');
    ...
    createPosts : function(callback) {
      ...
      async.eachLimit(fetcher.tweets, 1, function(tweet, nextTweet) {
        logger.log('Processing tweet ' + tweet.id_str);
        if (tweet.entities.urls.length === 0) {
          logger.log('Tweet has no url, trying next Tweet...');
          return nextTweet();
        }
        ...
        async.eachLimit(tweet.entities.urls, 1, function(url, nextURL) {
          request(url , function(err, res, body) {
            if (err) {
              logger.log('Could not get ' + url);
              logger.error(err);
              logger.error(err.stack);
              logger.log('Trying next URL');
            ...
            Post.find(['source_url = ?', [longURL]], function(err, posts) {
              ...
              if (posts.length > 0) {
                logger.log('URL exists... trying next URL');
                return nextURL();
              }
              ...
              Scrapper.processUrl(longURL, res, function(err, result) {
                ...
                logger.log('Creating new Post with data:');
                ...
                post.save(function(err, postResult) {
                  ...
                  if (result.images.length > 0) {
                    ...
                    logger.log('Post saved without an image');
                    return nextURL();
                  };
                });
              });
