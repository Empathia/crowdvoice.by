...
var ACL = {
  ...
  //--------------------------------------------------------------
  allow : function(action, resource, role, assert){
    this.setRule(action, resource, role, assert, this.TYPE_ALLOW);
  },
  ...
  //--------------------------------------------------------------
  isAllowed : function(action, resource, role, assertArguments){
    var assertResult;

    var data = assertArguments[0];
    var callback = assertArguments[1];

    rule = this.getRule(action, resource, role, true);

    if(rule.assert){
      assertResult = rule.assert(this, assertArguments);
    }else{
      assertResult = true;
    }

    if(assertResult && rule.type  == this.TYPE_ALLOW){
      logger.log('Allowed: ACL rule of ' + rule.type + ' of action ' + action + ' on ' + resource + ' for ' + role);

      if (assertArguments) {
        logger.log('ACL Args:');
        logger.log(assertArguments);
      }

      return callback(null, true);
    }

    logger.log('Denied: ACL rule of ' + rule.type + ' of action ' + action + ' on ' + resource + ' for ' + role);
