...
ACL.allow('repost', 'posts', 'Visitor', function (acl, data, respond) {
  /**
   * data = {
   *   voicesIds: array of IDs
   * }
   */

  K.Voice.query()
    .select('type')
    .where('id', 'in', data.voicesIds.map(function (v) { return hashids.decode(v)[0]; }))
    .then(function (voiceTypes) {
      var types = voiceTypes.map(function (v) { return v.type; });

      return respond(null, types.indexOf(Voice.TYPE_CLOSED) === -1)
    })
    .catch(respond);
});
