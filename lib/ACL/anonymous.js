...
ACL.allow('requestToContribute', 'voices', 'Anonymous', function(acl, data, next) {
  return(null, {isAllowed : false});
});

ACL.allow(['create', 'update'], 'voices', 'Anonymous', function(acl, data, next) {
...
ACL.allow('removeEntityFromOrg', 'entities', 'Anonymous', function (acl, data, next) {
  EntityOwner.find({
...
ACL.allow('leaveOrganization', 'entities', 'Anonymous', function (acl, data, respond) {
  var decoded = {
  ...
  if (decoded.currentPersonId === decoded.entityId) {
    // yes, so check if the user is part of the organization he's trying to leave
