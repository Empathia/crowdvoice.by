...
ACL.allow(['preview', 'create', 'upload'], 'posts', 'Anonymous', function(acl, data, next) {
  ...
  async.series([
    ...
    if (realCurrentPerson.id === data.activeVoice.ownerId
      || postOwner.id === data.activeVoice.ownerId) {

      response.isVoiceOwner = true;
    }
...
ACL.allow(['update', 'destroy', 'uploadPostImage', 'deleteOlderThan', 'deleteAllUnmoderated'], 'posts', 'Anonymous', function(acl, data, next) {
  var currentPerson,
    voice,
    profile,
    isVoiceCollaborator = false,
...
ACL.allow(['destroy', 'edit', 'update', 'isVoiceSlugAvailable'], 'voices', 'Anonymous', function(acl, data, respond) {
