...
ACL.allow(['isProfileNameAvailable', 'isEmailAvailable', 'edit', 'update'], 'entities', 'Person', function(acl, data, next) {
  var isAllowed = false;
...
ACL.allow('reportEntity', 'entities', 'Person', function (acl, data, respond) {
  ...
  Report.find({
    reportedId: d.currentEntityId,
    reporterId: d.currentPersonId
  }, function (err, result) {
    if (err) { return respond(err); }

    if (result.length > 0) {
      return respond(new ForbiddenError('you have already made this report in the past'));
    }
...
ACL.allow('createOrganization', 'entities', 'Person', function (acl, data, respond) {
  // just by being a person he is allowed
  return respond(null, true);
});
