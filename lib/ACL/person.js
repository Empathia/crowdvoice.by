...
ACL.allow('destroy', 'threads', 'Person', function(acl, data, next) {
  K.MessageThread.query()
    ...
    .include('[senderPerson,receiverEntity.owner]')
    .then(function (result) {
      var thread = result[0]

      var isAllowed = false

      if (data.currentPersonId === thread.senderPerson.id) {
        isAllowed = true
      }

      if (data.currentPersonId === thread.receiverEntity.id) {
        isAllowed = true
      }
      console.log(thread.receiverEntity)


      if (thread.receiverEntity.owner.length > 0 &&
        data.currentPersonId === thread.receiverEntity.owner[0].id) {
